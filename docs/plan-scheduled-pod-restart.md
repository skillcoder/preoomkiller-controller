...
RestartScheduleJitterMax     time.Duration  // env PREOOMKILLER_RESTART_SCHEDULE_JITTER_MAX, default 30s; units s/m/h
```
...
- New env vars: `PREOOMKILLER_ANNOTATION_RESTART_SCHEDULE`, `PREOOMKILLER_ANNOTATION_TZ`,
  `PREOOMKILLER_RESTART_SCHEDULE_JITTER_MAX` (duration with units s/m/h). The `restart-at` annotation key
